    ...
    for i, folder in enumerate(audio_path):
        ...
        for utter_name in os.listdir(folder):
            if utter_name[-4:] == '.wav':
                ...
                # intervals = librosa.effects.split(utter, top_db=40)  # voice activity detection
                # for interval in intervals:
                # if (interval[1] - interval[0]) > utter_min_len:  # If partial utterance is sufficient long,
                # utter_part = utter[interval[0]:interval[1]]  # save first and last 180 frames of spectrogram.
                S = librosa.core.stft(y=utter, n_fft=hp.data.nfft) # change 12.2
                S = np.abs(S) ** 2
                mel_basis = librosa.filters.mel(sr=hp.data.sr/4, n_fft=hp.data.nfft, n_mels=hp.data.nmels) # change 12.2
                S = np.log10(np.dot(mel_basis, S) + 1e-6)  # log mel spectrogram of utterances
                # print(S.shape)
                S = librosa.feature.mfcc(S=S, sr=sr, n_mfcc=40)
