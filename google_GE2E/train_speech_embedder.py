...
def test(model_path):
    ...
    avg_EER = 0
    f = open('test_result_Ultsound7', 'w')
    for e in range(hp.test.epochs):
        ...
        for batch_id, mel_db_batch in enumerate(test_loader):
            ...
            print(enrollment_embeddings.size())
            ...
            for thres in [0.01 * i + 0.5 for i in range(50)]:
                print("current thres is {}".format(thres))
                sim_matrix_thresh = sim_matrix > thres
                print(sim_matrix_thresh)
                ...
                if diff > abs(FAR - FRR):
                    ...
                    EER_FRR = FRR
            batch_avg_EER += EER
            print("\nEER : %0.2f (thres:%0.2f, FAR:%0.2f, FRR:%0.2f)" % (EER, EER_thresh, EER_FAR, EER_FRR), file=f)
            print("\nEER : %0.2f (thres:%0.2f, FAR:%0.2f, FRR:%0.2f)" % (EER, EER_thresh, EER_FAR, EER_FRR))
        avg_EER += batch_avg_EER / (batch_id + 1)
    avg_EER = avg_EER / hp.test.epochs
    print("\n EER across {0} epochs: {1:.4f}".format(hp.test.epochs, avg_EER))
    print("\n EER across {0} epochs: {1:.4f}".format(hp.test.epochs, avg_EER), file=f)
